...
final class VIS_Event_Bus {
    ...
    public static function ingest(array $event): void {
        ...
        $safeContext = self::scrub_context($context);

        // Astraea evidence bridge: emit only sanitized data, never raw secrets.
        if (function_exists('do_action')) {
            do_action('astraea_gedefense_security_event', [
                'module' => $module,
                'type' => $type,
                'message' => $message,
                'context' => $safeContext,
        ...
        if ($context !== []) {
            try {
                $encoded = wp_json_encode($safeContext, JSON_THROW_ON_ERROR);
                $message .= ' | ctx=' . $encoded;
            } catch (\Throwable $e) {
