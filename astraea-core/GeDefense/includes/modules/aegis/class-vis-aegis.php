...
final class VIS_Aegis {
    ...
    public function __construct(array $options = []) {
        if (empty($options)) {
            global $wpdb;
            if (isset($wpdb) && ($wpdb instanceof \wpdb) && !empty($wpdb->dbh) && function_exists('wp_cache_get') && function_exists('get_option')) {
        ...
        if (is_array($raw_ips)) {
            $this->whitelist_ips = array_filter(array_map('trim', $raw_ips), static function($ip) {
                return filter_var($ip, FILTER_VALIDATE_IP);
            });
        } else {
            $this->whitelist_ips = array_filter(array_map('trim', explode("\n", (string)$raw_ips)), static function($ip) {
                return filter_var($ip, FILTER_VALIDATE_IP);
            });
        }
    ...
    private static function get_nexus_signatures(): array {
        ...
        $threat_map = function_exists('wp_cache_get') ? wp_cache_get('vgt_nexus_matrix_map', 'vgt') : false;
        if ($threat_map === false) {
            $vault_dir = defined('WP_CONTENT_DIR') ? WP_CONTENT_DIR . '/vgt-vault/gorgon/' : dirname(ABSPATH) . '/wp-content/vgt-vault/gorgon/';
            $matrix_file = function_exists('wp_normalize_path') ? wp_normalize_path($vault_dir . 'nexus_matrix.json') : str_replace('\\', '/', $vault_dir . 'nexus_matrix.json');
            
            if (is_readable($matrix_file)) {
                $json_data = json_decode((string)file_get_contents($matrix_file), true);
                $threat_map = ($json_data['schema'] ?? '') === 'literal-v1' ? ($json_data['patterns'] ?? []) : [];
            } else {
                $threat_map = [];
            }
        ...
        if (is_array($threat_map)) {
            foreach ($threat_map as $name => $pattern_data) {
                if (($pattern_data['type'] ?? '') === 'literal' && !empty($pattern_data['signature'])) {
    ...
    private function scan_string_heuristic(string $input, string $context): void {
        ...
        foreach ($nexus_sigs as $threat_name => $pattern) {
            $runtimePattern = self::bounded_remote_regex($pattern);
            $match = $runtimePattern === '' ? false : @preg_match($runtimePattern, $input);
            if ($match === 1) {
                $this->terminate("Neural Swarm Threat detected in $context: [$threat_name]", 'BLOCK', $threat_name);
            } elseif ($match === false) {
                $last_error = preg_last_error();
                if ($last_error !== PREG_NO_ERROR) {
                    error_log("[SEC] Aegis: PCRE error {$last_error} in Nexus pattern [{$threat_name}]. Skipping faulty signature.");
                    continue;
                }
            }
        }
    ...
    private static function bounded_remote_regex(string $pattern): string {
        if (strlen($pattern) > 2048 || preg_match('/\A([^A-Za-z0-9\\s])(.*)\1([a-zA-Z]*)\z/s', $pattern, $parts) !== 1) {
            return '';
        }
        $body = (string)$parts[2];
        if (str_starts_with($body, '(*LIMIT_MATCH=5000)(*LIMIT_DEPTH=128)')) return $pattern;
        if (str_contains($body, '(*') || preg_match('/\\\\[1-9]|\(\?(?:R|0|&|P>)/i', $body) === 1) return '';
        return $parts[1] . '(*LIMIT_MATCH=5000)(*LIMIT_DEPTH=128)' . $body . $parts[1] . $parts[3];
    }
