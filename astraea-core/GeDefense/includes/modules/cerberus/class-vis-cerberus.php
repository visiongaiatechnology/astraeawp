...
final class VIS_Cerberus {
    ...
    public function enforce_global_perimeter(): void {
        ...
        if (class_exists('\\VIS_Bootstrapper') && \VIS_Bootstrapper::is_installing()) return;
        if (defined('WP_INSTALLING') && WP_INSTALLING) return;
        if (defined('WP_SETUP_CONFIG') && WP_SETUP_CONFIG) return;
    ...
    public function is_ip_banned(?string $custom_ip = null): bool {
        ...
        if (!empty($wpdb->last_error) && (str_contains($wpdb->last_error, "doesn't exist") || str_contains($wpdb->last_error, "1146"))) {
    ...
    public function unban_target(string $target): bool {
        ...
        if ($deleted === false) {
            ...
            throw new StorageException('Cerberus unban persistence failed.');
        }
