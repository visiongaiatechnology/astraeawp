...
final class VIS_Throne_Guard {
    ...
    public static function provision_current_master(): bool {
        ...
        if ($instance->has_master()) {
            return false;
        }
        $user = wp_get_current_user();
        if (!$user->exists() || !$user->has_cap('manage_options')) return false;
        return self::provision_user_as_master((int)$user->ID);
    }

    public static function provision_user_as_master(int $userId): bool {
        ...
        $success = $fresh instanceof WP_User && $fresh->has_cap(self::MASTER_CAP);
        if ($success) {
            self::log_event('MASTER_CLAIM', 'Benutzer [ID: ' . $userId . '] wurde als Astraea Master provisioniert.', 'success');
        }
        return $success;
    }

    public static function install_recovery_key(string $recoveryKey): bool {
        if (!defined('PASSWORD_ARGON2ID') || !in_array('argon2id', password_algos(), true)) return false;
        if (strlen($recoveryKey) < 48 || strlen($recoveryKey) > 96 || preg_match('/^ATG-[A-Z0-9_-]+(?:-[A-Z0-9_-]+)+$/D', $recoveryKey) !== 1) return false;
        $hash = password_hash($recoveryKey, PASSWORD_ARGON2ID);
        if (!is_string($hash) || !str_starts_with($hash, '$argon2id$')) return false;
        $saved = update_option('vis_throneguard_superkey_hash', $hash, false);
        if (!$saved && !hash_equals((string)get_option('vis_throneguard_superkey_hash', ''), $hash)) return false;
        return true;
    }
    ...
    private function ensure_master_role(): void {
        if (!function_exists('get_role')) {
            if (function_exists('add_action')) {
                add_action('init', [$this, 'ensure_master_role'], 1);
            }
            return;
        }
        ...
        if ($master === null) {
            add_role(self::MASTER_ROLE, 'Astraea Master', $capabilities);
            return;
        }
    ...
    public function handle_claim_master(): never {
        ...
        if ($this->has_master() || !self::provision_current_master()) {
            wp_die('Astraea Master role is already claimed or cannot be provisioned.', 'Security Error', ['response' => 403]);
        }
    ...
    public function handle_save(): never {
        ...
        if ($existingHash === '' || $newKey !== '') {
            if ($newKey !== '') {
                if (strlen($newKey) < 24 || strlen($newKey) > 256) {
                    wp_safe_redirect($this->dashboard_url(['throne_error' => 'key_length']));
                    exit;
                }
                $newHash = defined('PASSWORD_ARGON2ID') ? password_hash($newKey, PASSWORD_ARGON2ID) : false;
...
<body>
    <main class="tg-container">
        ...
        <div class="tg-footer">
            GEDEFENSE WP // THRONEGUARD // RECOVERY HASH: ARGON2ID
        </div>
    </main>
</body>
</html><?php
