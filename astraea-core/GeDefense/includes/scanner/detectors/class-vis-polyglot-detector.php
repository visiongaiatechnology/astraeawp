...
final class VIS_Polyglot_Detector implements VIS_File_Detector {
    ...
    public function detect(string $path, VIS_Scan_Context $context, VIS_Scan_Budget $budget): array {
        ...
        if ($context->expectedMime !== null && !$this->mimeMatches($context->expectedMime, $context->detectedMime)) {
            $findings[] = new VIS_Scan_Finding('MIME_EXTENSION_MISMATCH', 82, 98, 'Detected MIME type violates the extension policy.', false, VIS_Scan_Finding::CLASS_SUSPICIOUS);
        }
        ...
        if ($expectedType !== null) {
            ...
            if (!is_array($imageInfo) || !isset($imageInfo[2]) || $imageInfo[2] !== $expectedType) {
                $findings[] = new VIS_Scan_Finding('IMAGE_TYPE_POLYGLOT', 96, 98, 'Independent image type validation rejected the payload.', true, VIS_Scan_Finding::CLASS_MALWARE);
            }
        }

        $head = $this->readHead($path, 4096);
        if ($context->detectedMime === 'application/pdf' && !str_starts_with($head, '%PDF-')) {
            $findings[] = new VIS_Scan_Finding('INVALID_PDF_MAGIC', 78, 98, 'PDF magic bytes are missing.', false, VIS_Scan_Finding::CLASS_SUSPICIOUS);
        }
        if ($context->detectedMime === 'application/zip' && !str_starts_with($head, "PK\x03\x04") && !str_starts_with($head, "PK\x05\x06")) {
            $findings[] = new VIS_Scan_Finding('INVALID_ZIP_MAGIC', 78, 98, 'ZIP magic bytes are missing.', false, VIS_Scan_Finding::CLASS_SUSPICIOUS);
        }
        return $findings;
    }
